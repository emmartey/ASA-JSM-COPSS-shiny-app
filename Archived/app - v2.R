  ...
  nav_panel(
    ...
    layout_sidebar(
      sidebar = sidebar(
        ...
        sliderInput("copss_year_rng", "Year range", min = 1964, max = 2040, value = c(2000, 2026), sep = ""),
...
server <- function(input, output, session) {
  ...
  raw_data <- eventReactive(input$reload, {
    ...
    list(
      copss1 = safe_read_sheet(SHEET_COPSS, #input$copss_tab
                              "Sheet1"
                              ),
      copss2 = safe_read_sheet(SHEET_COPSS, #input$copss_tab
                              "Sheet2"),
